...
depends_on: ["07-01"]
autonomous: true
requirements_addressed: ["POLY-03"]
files_modified:
...
<acceptance_criteria>
- `apps/backend/scripts/smoke_test.py` exists with 20 questions across 4 categories
- Each question is tested in all 3 modes (Normal, RAG, Coach) — 60 total calls
- Script runs with `python apps/backend/scripts/smoke_test.py` and exits 0 on all pass
- Script exits non-zero if any question fails
- `grep -c "def test_" apps/backend/scripts/smoke_test.py` returns 20
- Script measures GPU memory before/after via `/health/gpu`
- Output includes pass/fail per question and summary table
</acceptance_criteria>

<action>

**File: `apps/backend/scripts/smoke_test.py`** (NEW FILE):
...
GPU memory is checked before and after via /health/gpu.
"""
...
async def run_question(
    ...
    payload = {
        ...
        "mode": mode,
        "session_id": None,  # fresh session per question
        "stream": False,
    }
    start = time.time()
    try:
...
def print_summary(result: SmokeTestResult) -> None:
    ...
    for r in result.results:
        ...
        print(
            ...
            f"{marker} {status_str:<5} {char_count:<20} {preview}"
        )
    print()
...
async def main() -> int:
    ...
    print(f"Backend: {BACKEND_BASE}")
    print(f"Questions: {len(QUESTIONS)} × {len(MODES)} modes = {len(QUESTIONS) * len(MODES)} calls")
    print()
...
<acceptance_criteria>
- `python apps/backend/scripts/smoke_test.py --help` or just `python apps/backend/scripts/smoke_test.py` runs without import errors
- `grep -c "def test_\|Question(" apps/backend/scripts/smoke_test.py` returns >= 20 question definitions
- All 3 modes are tested for each question (grep finds "normal", "rag", "coach")
- Exit code 0 when all pass, non-zero when any fail
- `smoke_test_results.json` is created after running
</acceptance_criteria>
...
```bash
cd apps/backend
python scripts/smoke_test.py 2>&1 | head -50
# Should show: question headers, pass/fail per call, summary table
```

Note: This script requires the backend to be running at `http://localhost:8000`. If backend is not running, the script will timeout or fail gracefully (non-blocking per question — it tries all 60 calls and reports failures).

Check for correct question count:

```bash
grep -c "^    Question(" apps/backend/scripts/smoke_test.py
# Expected: 20
```

Check all modes are included:

```bash
grep '"normal"\|"rag"\|"coach"' apps/backend/scripts/smoke_test.py | wc -l
# Expected: 3
```
...
1. Script runs and reports pass/fail for all 60 (20 × 3) calls
2. GPU memory is recorded before and after the test run
3. `smoke_test_results.json` contains structured data for all 60 results
4. Exit code 0 when all pass, non-zero when any fail

**Must-haves (goal-backward):**
- 20 questions cover all 4 categories (comp/item/augment/pivot)
- All 3 modes (Normal/RAG/Coach) are tested for every question
- GPU memory < 16GB throughout the test run
